...
Usage:
    ...
    python -m src.models.fusion.tune_late_fusion --n_trials 30 --modalities ehr img txt

    # Tune Option B (Pre-trained)
    python -m src.models.fusion.tune_late_fusion --n_trials 30 --load_pretrained --modalities ehr img txt
"""
...
from src.models.fusion.train_late_fusion import load_pretrained_unimodal_weights
...
def objective(
    ...
    input_dims = {k: base_input_dims[k] for k in active_modalities}
...
def main():
    ...
    # Save to a dynamically named JSON file so we don't overwrite the 4-mod baseline tuning
    suffix = (
        f"_{len(args.modalities)}mod_pretrained"
        if args.load_pretrained
        else f"_{len(args.modalities)}mod"
    )
    ...
    train_dataset = MultimodalSepsisDataset(
        split="train", active_modalities=args.modalities
    )
    val_dataset = MultimodalSepsisDataset(
        split="valid", active_modalities=args.modalities
    )
    ...
    logger.info(
        f"Starting Optuna study for {len(args.modalities)}-MOD LATE FUSION [{mode_name}] with {args.n_trials} trials..."
    )
